...
This is a learning project — when helping, explain the *why* behind neural net
concepts, don't just write code. The user wants to understand, not just have a
working result. Keep the from-scratch NumPy style (no PyTorch in phase 1).
Network is [5, 8, 1] feedforward for evolution and [5, 8, 3] for the DQN (one
Q-value per action). Both phases work: evolution in train_evolution.py, DQN in
train_rl.py.
